...
while IFS= read -r case_json; do
    ...
    # expectedOutputs key by key. jq does the comparison so numbers stay
    # numbers; a missing key surfaces as null, which is a mismatch unless null
    # is what the case expects.
    mismatches=$(jq -n \
      --argjson exp "$expected_outputs" \
      --argjson res "$response" \
      '($res | if type == "array" then (.[0] // {}) else . end
             | with_entries(.value |= (if type == "object" and has("value")
                                       then .value else . end))) as $act
       | [ $exp | to_entries[]
           | select((($act[.key]) // null) != .value)
           | "\(.key)(expected=\(.value|tostring),actual=\((($act[.key]) // null)|tostring))" ]
       | join(" ")' 2>/dev/null)
