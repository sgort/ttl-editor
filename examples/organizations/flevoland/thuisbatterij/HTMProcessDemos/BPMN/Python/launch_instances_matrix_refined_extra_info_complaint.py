...
#   afronden -> bezwaar after complaintStart days when willComplain=true
#   bezwaar -> end after complaintDuration days
#   afronden -> end after 10 days when willComplain=false
#
...
# through beoordeling cannot request extra information again. The script controls when
# afronden and bezwaar are completed so those states remain visible on the dashboard.
#
# Dry run, instant:
...
# python .\launch_instances_matrix_refined_extra_info_complaint.py --tenant 48 --count 1000 --prob-needs-extra-information 1 --prob-will-complain 1 --username YOUR_USER --password YOUR_PASSWORD --execute --heartbeat 0
...
DEFAULT_AFRONDEN_TOPIC = "afronden"
DEFAULT_BEZWAAR_TOPIC = "bezwaar"
...
class ProcessMatrixRow:
    ...
    @property
    def bezwaar_day(self) -> Optional[int]:
        if not self.will_complain:
            return None
        return self.afronden_day + self.complaint_start

    @property
    def end_day(self) -> int:
        if self.will_complain:
            assert self.bezwaar_day is not None
            return self.bezwaar_day + self.complaint_duration
        return self.afronden_day + self.afronden_without_complaint_wait_days

    afronden_without_complaint_wait_days: int = 10
...
def build_variable_payload(index: int, args: argparse.Namespace) -> VariablePayload:
    ...
    complaint_start = random.randint(
        args.min_complaint_start_days,
        args.max_complaint_start_days,
    )

    complaint_duration = random.randint(
        args.min_complaint_duration_days,
        args.max_complaint_duration_days,
    )
    ...
    return {
        ...
        "complaintStart": variable(complaint_start, "Long"),
        "complaintDuration": variable(complaint_duration, "Long"),
        "afrondenWithoutComplaintWait": variable(args.afronden_without_complaint_wait_days, "Long"),
    }
...
def build_process_matrix(args: argparse.Namespace) -> List[ProcessMatrixRow]:
    ...
    for index in range(1, args.count + 1):
        ...
        will_complain = bool(variables["willComplain"]["value"])
        ...
        complaint_start = int(variables["complaintStart"]["value"])
        complaint_duration = int(variables["complaintDuration"]["value"])
        ...
        row = ProcessMatrixRow(
            ...
            afronden_without_complaint_wait_days=args.afronden_without_complaint_wait_days,
        )
        ...
        row.transitions.append(
            TransitionPlan(
                ...
                completion_variables={
                    ...
                    "afrondenStart": variable(final_day, "Long"),
                    "needsExtraInformation": variable(False, "Boolean"),
                },
            )
        )

        if row.will_complain:
            bezwaar_day = row.bezwaar_day
            assert bezwaar_day is not None
            row.transitions.append(
                TransitionPlan(
                    name="afronden_to_bezwaar",
                    from_topic=args.afronden_topic,
                    target_state="bezwaar",
                    due_day=bezwaar_day,
                    completion_variables={
                        "willComplain": variable(True, "Boolean"),
                        "complaintRequestedDay": variable(bezwaar_day, "Long"),
                        "bezwaarStart": variable(bezwaar_day, "Long"),
                    },
                )
            )
            row.transitions.append(
                TransitionPlan(
                    name="bezwaar_to_end",
                    from_topic=args.bezwaar_topic,
                    target_state="end",
                    due_day=row.end_day,
                    completion_variables={
                        "complaintCompleted": variable(True, "Boolean"),
                        "complaintEnd": variable(row.end_day, "Long"),
                        "processEndDay": variable(row.end_day, "Long"),
                    },
                )
            )
        else:
            row.transitions.append(
                TransitionPlan(
                    name="afronden_to_end_no_complaint",
                    from_topic=args.afronden_topic,
                    target_state="end",
                    due_day=row.end_day,
                    completion_variables={
                        "willComplain": variable(False, "Boolean"),
                        "processEndDay": variable(row.end_day, "Long"),
                    },
                )
            )
...
def matrix_to_dict(row: ProcessMatrixRow) -> Dict[str, Any]:
    return {
        ...
        "willComplain": row.will_complain,
        "complaintStart": row.complaint_start,
        "complaintDuration": row.complaint_duration,
        "bezwaarDay": row.bezwaar_day,
        "endDay": row.end_day,
...
def print_matrix(matrix: List[ProcessMatrixRow]) -> None:
    ...
    for row in matrix:
        ...
        bezwaar_day = row.bezwaar_day if row.bezwaar_day is not None else "-"
        ...
        print(
            ...
            f"afrondenDay={row.afronden_day:>3} "
            f"bezwaarDay={str(bezwaar_day):>3} "
            f"endDay={row.end_day:>3} "
            f"needsExtraInformation={row.needs_extra_information} "
            f"willComplain={row.will_complain} "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Minimal-REST matrix heartbeat runner with one-time BPMN extra-information loop and complaint handling for Operaton/Camunda 7."
    )
    ...
    parser.add_argument(
        "--afronden-topic",
        default=DEFAULT_AFRONDEN_TOPIC,
        help="External task topic to complete when leaving afronden. Default: afronden",
    )
    parser.add_argument(
        "--bezwaar-topic",
        default=DEFAULT_BEZWAAR_TOPIC,
        help="External task topic to complete when leaving bezwaar. Default: bezwaar",
    )
    ...
    parser.add_argument("--min-complaint-start-days", type=int, default=2)
    parser.add_argument("--max-complaint-start-days", type=int, default=3)
    parser.add_argument("--min-complaint-duration-days", type=int, default=3)
    parser.add_argument("--max-complaint-duration-days", type=int, default=5)
    parser.add_argument(
        "--afronden-without-complaint-wait-days",
        type=int,
        default=10,
        help="Number of simulated days to keep no-complaint cases at afronden before ending. Default: 10",
    )
    parser.add_argument(
    ...
    range_checks = [
        ...
        ("--min-complaint-start-days", args.min_complaint_start_days, "--max-complaint-start-days", args.max_complaint_start_days),
        ("--min-complaint-duration-days", args.min_complaint_duration_days, "--max-complaint-duration-days", args.max_complaint_duration_days),
    ]
    ...
    if args.afronden_without_complaint_wait_days < 0:
        print("--afronden-without-complaint-wait-days cannot be negative")
        sys.exit(1)
...
def main() -> None:
    ...
    print(f"  complaint transition: {args.afronden_topic} -> bezwaar after complaintStart days when willComplain=true")
    print(f"  complaint end transition: {args.bezwaar_topic} -> end after complaintDuration days")
    print(f"  no-complaint end transition: {args.afronden_topic} -> end after {args.afronden_without_complaint_wait_days} days")
    print(f"  auto task fallback: {args.auto_task_fallback}")
