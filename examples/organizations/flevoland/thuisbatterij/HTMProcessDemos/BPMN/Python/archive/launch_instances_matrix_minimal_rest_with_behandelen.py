# launch_instances_matrix_minimal_rest.py
#
...
#
# Extra-info flow now includes:
#   beoordeling -> informatie on originalSubmissionStart + submissionDuration + extraInformationStart
#   informatie -> behandelen on originalSubmissionStart + submissionDuration + extraInformationStart + extraInformationDuration
#
# Dry run, instant:
#   python .\launch_instances_matrix_minimal_rest.py --tenant 48 --count 10 --seed 123 --show-matrix --no-sleep
#
# Execute with a 20-second heartbeat:
#   python .\launch_instances_matrix_minimal_rest.py --tenant 48 --count 10 --username YOUR_USER --password YOUR_PASSWORD --execute
...
DEFAULT_INFORMATION_TOPIC = "informatie"
...
class ProcessMatrixRow:
    ...
    @property
    def behandelen_day(self) -> Optional[int]:
        if not self.needs_extra_information:
            return None
        return (
...
class OperatonClient:
    ...
    def query_unlocked_task_by_instance_id(self, process_instance_id: str, topic_name: str) -> Optional[Dict[str, Any]]:
        params = {
...
def build_variable_payload(index: int, args: argparse.Namespace) -> VariablePayload:
    ...
    # User rule:
    # if needsExtraInformation is true, evaluationDuration must always be at least
...
def build_process_matrix(args: argparse.Namespace) -> List[ProcessMatrixRow]:
    ...
    for index in range(1, args.count + 1):
        ...
        if needs_extra_information:
            ...
            behandelen_day = row.behandelen_day
            assert behandelen_day is not None
            row.transitions.append(
                TransitionPlan(
                    name="informatie_to_behandelen",
                    from_topic=args.behandelen_from_topic,
                    target_state="behandelen",
                    due_day=behandelen_day,
                    completion_variables={
                        "treatmentStart": variable(behandelen_day, "Long"),
                    },
                )
            )
...
def matrix_to_dict(row: ProcessMatrixRow) -> Dict[str, Any]:
    return {
        ...
        "behandelenDay": row.behandelen_day,
...
def print_matrix(matrix: List[ProcessMatrixRow]) -> None:
    ...
    for row in matrix:
        ...
        behandelen_day = row.behandelen_day if row.behandelen_day is not None else "-"
        ...
        print(
            ...
            f"behandelenDay={str(behandelen_day):>3} "
            ...
            writer.writerow(
                {
                    ...
                    "behandelenDay": row.behandelen_day or "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Minimal-REST matrix heartbeat runner for Operaton/Camunda 7."
    )
    ...
    parser.add_argument(
        ...
        help=(
            "External task topic to complete when moving from informatie to behandelen. "
            "Default: informatie"
        ),
    )
...
def main() -> None:
    ...
    print(f"  treatment transition: {args.behandelen_from_topic} -> behandelen")
