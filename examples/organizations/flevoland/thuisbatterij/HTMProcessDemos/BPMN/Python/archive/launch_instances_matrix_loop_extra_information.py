...
#
# Extra-info flow now includes a BPMN loop:
#   beoordeling -> informatie on originalSubmissionStart + submissionDuration + extraInformationStart
#   informatie -> beoordeling on originalSubmissionStart + submissionDuration + extraInformationStart + extraInformationDuration
#   beoordeling -> beoordeling afgerond on originalSubmissionStart + submissionDuration + evaluationDuration
#
# The BPMN must have sequence flow informatie -> beoordeling. Completing topic extra-informatie
# sets needsExtraInformation=false so the second pass through beoordeling can finish.
#
# Dry run, instant:
...
class ProcessMatrixRow:
    ...
    @property
    def beoordeling_afgerond_day(self) -> int:
...
def build_process_matrix(args: argparse.Namespace) -> List[ProcessMatrixRow]:
    ...
    for index in range(1, args.count + 1):
        ...
        final_day = row.beoordeling_afgerond_day
        row.transitions.append(
            TransitionPlan(
                name="beoordeling_to_afgerond",
                from_topic=args.extra_information_from_topic,
                target_state="beoordeling afgerond",
                due_day=final_day,
                completion_variables={
...
def matrix_to_dict(row: ProcessMatrixRow) -> Dict[str, Any]:
    return {
        ...
        "beoordelingAfgerondDay": row.beoordeling_afgerond_day,
...
def print_matrix(matrix: List[ProcessMatrixRow]) -> None:
    ...
    for row in matrix:
        ...
        print(
            ...
            f"finishDay={row.beoordeling_afgerond_day:>3} "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Minimal-REST matrix heartbeat runner with BPMN extra-information loop for Operaton/Camunda 7."
    )
...
def main() -> None:
    ...
    print(f"  final evaluation transition: {args.extra_information_from_topic} -> beoordeling afgerond")
    print(f"  auto task fallback: {args.auto_task_fallback}")
