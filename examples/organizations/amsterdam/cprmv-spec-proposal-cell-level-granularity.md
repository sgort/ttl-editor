...
| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-15 | Wrote the test suite (40 cases, 14 decisions, MC/DC on the 10 leaf decisions + 4 composite/root integration cases) — every case run live, zero errors. Building it surfaced two more malformed rule-cell patterns: `>= X and < Y` isn't a valid unary-test range (needs interval notation, `[X..Y)`/`[X..Y]`) and a bare `not "string"` has the same missing-parens problem as `not -`. Both fixed and confirmed. Deployed `HvA_full_dmn_export-patched.dmn` to Operaton as the sole/single-truth deployment for this DRD, removing the original unpatched deployment. New: [`testCases/test-cases-hva-full-dmn-export.json`](testCases/test-cases-hva-full-dmn-export.json), [`testCases/test-cases-validation-hva.md`](testCases/test-cases-validation-hva.md). Full writeup in [`hva-full-dmn-export-feel-evaluation-fix.md`](hva-full-dmn-export-feel-evaluation-fix.md) |
| 2026-08-14 | Fixed the last blocker from the entry below: `not -` (44 occurrences, always `boolean` columns) and `not(null) -` (12, always `string` columns) are malformed FEEL rule-cell content — resolved to `false` and `not(null)` respectively, a type-consistent, mechanical fix confirmed by business logic (e.g. `bijzondere bijstand`'s "financiele draagkracht" really must be false to qualify). Confirmed live with a clean true/false discrimination. All 25 decisions in `HvA_full_dmn_export-patched.dmn` should now evaluate correctly end to end                                                                                                                                                                                                                                                                                                                        |
| 2026-08-14 | Found and fixed the actual evaluate-blocker left open by the entry below: `<dmn:output>` needs a `name` attribute, not just `label` — Operaton's result serialization throws an unlogged, blank exception without it, but only once a decision table's inputs all resolve (so it was masked by the multi-word-name defect the whole time). Confirmed live: 4 leaf decisions now evaluate to the correct value, a 3-level-deep composite decision chains correctly. Also nailed down the REST `Date` type/format this DMN's date inputs need. Surfaced one more genuine defect while confirming the fix — `not -` (44 occurrences) isn't valid FEEL — left unfixed, flagged for a decision on the correct rewrite. `hva-full-dmn-export-feel-evaluation-fix.md` updated with all of the above                                                                                 |
| 2026-08-14 | `HvA_full_dmn_export.dmn` deploys but can't be evaluated — Operaton's FEEL engine can't parse this DMN's multi-word bare-name convention, plus a rule-cell cross-decision-reference naming collision and three broken/placeholder expressions. Root causes, empirical evidence, and the fix (`HvA_full_dmn_export-patched.dmn`) documented in `hva-full-dmn-export-feel-evaluation-fix.md` — no change to this doc's own content or worked examples                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 2026-08-13 | `HvA_full_dmn_export.dmn` fixed to deploy on Operaton: added `camunda:historyTimeToLive` to all 25 decisions, escaped 48 unescaped `&` in `knowledgeSource` URLs (invalid XML, independent of the TTL issue) — no change to this doc's own content or worked examples                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
