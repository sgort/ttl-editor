...
see #52. Every migration PR touches build config, so none of them can be filtered
out. **Resolve #52, or thin the Renovate queue, before starting.** Note only
`audit` is a required check, so a refused preview does not block a merge — but it
does remove the preview this migration most needs.
